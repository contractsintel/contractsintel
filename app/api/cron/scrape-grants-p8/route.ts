...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(API, {
      ...
      body: JSON.stringify({ keyword: "", oppStatuses: "posted", sortBy: "openDate|desc", rows: 100, offset: 700 }),
    ...
    return NextResponse.json({ success: true, page: 8, offset: 700, fetched: opps.length, saved, total: data.hitCount ?? 0 });
