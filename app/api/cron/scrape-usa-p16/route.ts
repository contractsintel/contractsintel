  ...
  try {
    ...
    const res = await fetch("https://api.usaspending.gov/api/v2/search/spending_by_award/", {
      ...
      body: JSON.stringify({
        ...
        limit: 100, page: 16, sort: "Award Amount", order: "desc", subawards: false
      }),
    ...
    for (const a of awards) {
      ...
      const { error } = await supabase.from("opportunities").upsert({
        ...
        agency: agency || "Unknown", solicitation_number: awardId,
        naics_code: a["NAICS Code"] ?? null, value_estimate: a["Award Amount"] ?? null,
        ...
        incumbent_name: a["Recipient Name"] ?? null, incumbent_value: a["Award Amount"] ?? null,
      }, { onConflict: "notice_id" });
      if (!error) saved++;
    }
    return NextResponse.json({ success: true, page: 16, fetched: awards.length, saved, hasNext: data.page_metadata?.hasNext ?? false });
