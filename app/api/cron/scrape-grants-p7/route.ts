...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(API, {
      ...
      body: JSON.stringify({ keyword: "", oppStatuses: "posted", sortBy: "openDate|desc", rows: 100, offset: 600 }),
    ...
    return NextResponse.json({ success: true, page: 7, offset: 600, fetched: opps.length, saved, total: data.hitCount ?? 0 });
