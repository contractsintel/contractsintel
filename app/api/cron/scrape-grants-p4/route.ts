...
export async function GET(request: NextRequest) {
  try {
    ...
    const res = await fetch(API, {
      ...
      body: JSON.stringify({ keyword: "", oppStatuses: "posted", sortBy: "openDate|desc", rows: 100, offset: 300 }),
    ...
    return NextResponse.json({ success: true, page: 4, offset: 300, fetched: opps.length, saved, total: data.hitCount ?? 0 });
