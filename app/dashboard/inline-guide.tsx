...
const GUIDES: Record<string, string> = {
  dashboard:
    "Your matched opportunities appear here every morning at 7am. Review the scores (higher = better fit), read the AI recommendation, then click Track, Mark as Bidding, or Skip on each one.",
  pipeline:
...
export function InlineGuide({ page }: { page: string }) {
  ...
  return (
    <div
      className="mb-5 border border-[#1e2535] bg-[#111520]"
      style={{ borderRadius: "10px", overflow: "hidden" }}
    >
      <button
        onClick={handleToggle}
        className="w-full flex items-center justify-between px-6 py-4 text-left hover:bg-[#161b2e] transition-colors"
      >
