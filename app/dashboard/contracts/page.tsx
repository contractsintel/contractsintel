          ...
          <div className="space-y-3 mt-6">
            {[1,2,3].map(i => (
              <div key={i} className="border border-[#1e2535] bg-white p-5">
                ...
                <div className="h-2 w-1/4 bg-[#111520] rounded" />
              </div>
            ))}
          </div>
        </div>
    ...
    <div>
      ...
      <div className="flex items-center gap-2 mb-5">
        <button
          ...
          className={`px-3 py-1.5 text-xs font-medium border ${view === "list" ? "bg-[#111827] text-white border-[#111827]" : "bg-white text-[#8b9ab5] border-[#1e2535] hover:border-[#2a3548]"}`}
        >
          List
        </button>
        <button
          onClick={() => setView("calendar")}
          className={`px-3 py-1.5 text-xs font-medium border ${view === "calendar" ? "bg-[#111827] text-white border-[#111827]" : "bg-white text-[#8b9ab5] border-[#1e2535] hover:border-[#2a3548]"}`}
        >
          Calendar
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="max-w-[480px] mx-auto text-center p-12" style={{background: "linear-gradient(135deg, #ecfeff, #fff)", borderRadius: "16px", border: "1px solid rgba(8,145,178,0.08)"}}>
          ...
          <p className="text-sm text-[#8b9ab5] mb-6">When you win a contract in the Pipeline, your delivery dashboard activates automatically with milestones, invoices, and deadline tracking.</p>
        ...
        <div className="space-y-6">
          {contracts.map((contract) => (
            <div key={contract.id} className="border border-[#1e2535] bg-white">
              {/* Contract Header */}
              ...
              {contract.milestones && contract.milestones.length > 0 && (
                <div className="p-5 border-b border-[#1e2535]">
                  <h4 className="text-[10px] font-medium uppercase tracking-wide text-[#4a5a75] mb-3">
                    Milestones
                  </h4>
              ...
              {contract.invoices && contract.invoices.length > 0 && (
                <div className="p-5">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="text-[10px] font-medium uppercase tracking-wide text-[#4a5a75]">
                    ...
                    <tbody>
                      {contract.invoices.map((inv: any) => (
                        <tr key={inv.id} className="border-t border-[#1e2535]">
                          <td className="py-2 text-xs text-[#e8edf8] font-mono">{inv.invoice_number || inv.number || "—"}</td>
      ...
      {demandLetter && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-full max-w-2xl border border-[#1e2535] bg-white p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="bg-[#080a0f] border border-[#1e2535] p-6 mb-4">
...
function ContractsCalendarView({
  ...
  return (
    <div className="border border-[#1e2535] bg-white rounded-xl">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-[#1e2535]">
        <div className="flex items-center gap-2">
          <button onClick={goPrev} className="px-2 py-1 text-xs border border-[#1e2535] hover:border-[#2a3548] rounded">
            ←
          </button>
          <button onClick={goToday} className="px-3 py-1 text-xs border border-[#1e2535] hover:border-[#2a3548] rounded">
            Today
          </button>
          <button onClick={goNext} className="px-2 py-1 text-xs border border-[#1e2535] hover:border-[#2a3548] rounded">
            →
          </button>
          <h3 className="text-sm font-semibold text-[#e8edf8] ml-2">{monthLabel}</h3>
        </div>
        <div className="flex items-center gap-3 text-[10px]">
          {(["deliverable", "report", "invoice", "option"] as const).map((t) => (
            <div key={t} className="flex items-center gap-1.5">
              ...
              <span className="text-[#8b9ab5] capitalize">{t}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 border-b border-[#1e2535]">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
          <div key={d} className="text-[10px] font-medium uppercase tracking-wide text-[#4a5a75] px-2 py-2 text-center">
            {d}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 grid-rows-6">
        {cells.map((cell, i) => {
          ...
          return (
            <div
              key={i}
              className={`min-h-[88px] border-r border-b border-[#1e2535] p-1.5 ${
                cell.date ? "" : "bg-[#fafbfc]"
              }`}
            >
              {cell.date && (
                <div className={`text-[11px] font-mono mb-1 ${isToday ? "text-[#2563eb] font-bold" : "text-[#4a5a75]"}`}>
                  {cell.date.getDate()}
                </div>
              )}
              <div className="space-y-0.5">
                ...
                {cell.events.length > 3 && (
                  <div className="text-[10px] text-[#8b9ab5] px-1.5">+{cell.events.length - 3} more</div>
                )}
              </div>
            </div>
          );
