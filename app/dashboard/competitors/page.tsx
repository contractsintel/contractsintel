...
export default function CompetitorsPage() {
  ...
  return (
    <div>
        ...
        <div className="border border-[#e5e7eb] bg-white p-5 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] mb-6">
          <form onSubmit={handleAdd} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-[#64748b] mb-1 font-medium uppercase tracking-wide">Company Name</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-xs text-[#64748b] mb-1 font-medium uppercase tracking-wide">UEI (optional)</label>
                <input
                  type="text"
                  value={form.uei}
            ...
            <div>
              <label className="block text-xs text-[#64748b] mb-1 font-medium uppercase tracking-wide">Notes</label>
              <textarea
                value={form.notes}
      ...
      {/* Competitors List */}
      {loading ? (
        <div className="text-center text-[#94a3b8] py-12">Loading competitors...</div>
      ) : competitors.length === 0 ? (
        <div className="border border-[#e5e7eb] bg-white p-12 text-center">
          <div className="text-[#94a3b8] text-lg mb-2">No competitors tracked</div>
          <p className="text-sm text-[#64748b]">Add competitors to start building intelligence.</p>
        </div>
      ) : (
              ...
              <div key={comp.id} className="border border-[#e5e7eb] bg-white">
                <div className="p-5 border-b border-[#e5e7eb]">
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-sm text-[#0f172a] font-medium">{comp.name}</h3>
                      {comp.uei && <p className="text-xs font-mono text-[#94a3b8] mt-0.5">UEI: {comp.uei}</p>}
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        ...
                        <span className="text-xs text-[#94a3b8]">{encounters.length} encounters</span>
                      </div>
                    </div>
                    <button
                  ...
                  {comp.notes && <p className="text-sm text-[#64748b] mt-3">{comp.notes}</p>}
                </div>
                        ...
                        {visible.map((enc: any) => (
                          <div key={enc.id} className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-[#0f172a]">{enc.opportunity_title ?? "Unknown"}</span>
                            </div>
                            <span className={`text-[10px] font-mono uppercase ${
                              enc.outcome === "win" ? "text-[#22c55e]" : enc.outcome === "loss" ? "text-[#ef4444]" : "text-[#94a3b8]"
                            }`}>
                              {enc.outcome ?? "pending"}
                            </span>
                          </div>
