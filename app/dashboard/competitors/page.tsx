...
export default function CompetitorsPage() {
      ...
      <div className="relative">
        <div className="absolute inset-0 z-10 flex items-center justify-center">
          <div className="border border-[#1e2535] bg-[#0d1018] p-8 max-w-md text-center">
            ...
            <h2 className="text-lg font-serif text-[#e8edf8] mb-2">Competitor Intelligence — Team Feature</h2>
        ...
        <div className="filter blur-sm opacity-40 pointer-events-none">
          <h1 className="text-2xl font-serif text-[#e8edf8] mb-6">Competitor Intelligence</h1>
  ...
  return (
    <div>
        ...
        <div className="border border-[#1e2535] bg-[#0d1018] p-5 mb-6">
          <form onSubmit={handleAdd} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-[#8b9ab5] mb-1 font-mono uppercase tracking-wider">Company Name</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-xs text-[#8b9ab5] mb-1 font-mono uppercase tracking-wider">UEI (optional)</label>
                <input
                  type="text"
                  value={form.uei}
            ...
            <div>
              <label className="block text-xs text-[#8b9ab5] mb-1 font-mono uppercase tracking-wider">Notes</label>
              <textarea
                value={form.notes}
      ...
      {/* Competitors List */}
      {loading ? (
        <div className="text-center text-[#4a5a75] py-12">Loading competitors...</div>
      ) : competitors.length === 0 ? (
        <div className="border border-[#1e2535] bg-[#0d1018] p-12 text-center">
          <div className="text-[#4a5a75] text-lg mb-2">No competitors tracked</div>
          <p className="text-sm text-[#8b9ab5]">Add competitors to start building intelligence.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {competitors.map((comp) => {
            ...
            return (
              <div key={comp.id} className="border border-[#1e2535] bg-[#0d1018]">
                <div className="p-5 border-b border-[#1e2535]">
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-sm text-[#e8edf8] font-medium">{comp.name}</h3>
                      {comp.uei && <p className="text-xs font-mono text-[#4a5a75] mt-0.5">UEI: {comp.uei}</p>}
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        ...
                        <span className="text-xs text-[#4a5a75]">{encounters.length} encounters</span>
                      </div>
                    </div>
                    <button
                  ...
                  {comp.notes && <p className="text-sm text-[#8b9ab5] mt-3">{comp.notes}</p>}
                </div>

                {/* Encounters */}
                {encounters.length > 0 && (
                  <div className="p-5">
                    <h4 className="text-[10px] font-mono uppercase tracking-wider text-[#4a5a75] mb-3">Encounters</h4>
                    <div className="space-y-2">
                      {encounters.slice(0, 5).map((enc: any) => (
                        <div key={enc.id} className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className={`w-2 h-2 ${
                              enc.outcome === "win" ? "bg-[#22c55e]" : enc.outcome === "loss" ? "bg-[#ef4444]" : "bg-[#4a5a75]"
                            }`} />
                            <span className="text-xs text-[#e8edf8]">{enc.opportunity_title ?? "Unknown"}</span>
                          </div>
                          <span className={`text-[10px] font-mono uppercase ${
                            enc.outcome === "win" ? "text-[#22c55e]" : enc.outcome === "loss" ? "text-[#ef4444]" : "text-[#4a5a75]"
                          }`}>
                            {enc.outcome ?? "pending"}
                          </span>
                        </div>
                      ...
                      {encounters.length > 5 && (
                        <p className="text-xs text-[#4a5a75]">+{encounters.length - 5} more</p>
                      )}
                    </div>
                  </div>
                )}
      ...
      {analysisView && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-full max-w-2xl border border-[#1e2535] bg-[#0d1018] p-6 max-h-[80vh] overflow-y-auto">
            <h2 className="text-lg font-serif text-[#e8edf8] mb-4">Competitive Analysis</h2>
