...
function ratingColor(value: string): string {
  switch (value) {
    ...
    case "Satisfactory": return "text-[#0f172a]";
          ...
          <div className="space-y-3 mt-6">
            {[1,2,3].map(i => (
              <div key={i} className="border border-[#e5e7eb] bg-white p-5">
                ...
                <div className="h-2 w-1/4 bg-[#f1f5f9] rounded mb-2" />
  ...
  return (
    <div>
      ...
      <div className="border border-[#e5e7eb] bg-white p-5 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] mb-6">
        <h2 className="text-[10px] font-medium uppercase tracking-wide text-[#94a3b8] mb-4">Add CPARS Rating</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-[#64748b] mb-1 font-medium uppercase tracking-wide">Contract</label>
              <select
            ...
            <div>
              <label className="block text-xs text-[#64748b] mb-1 font-medium uppercase tracking-wide">Category</label>
              <select
                value={form.category}
            ...
            <div>
              <label className="block text-xs text-[#64748b] mb-1 font-medium uppercase tracking-wide">Rating</label>
              <select
                value={form.rating}
            ...
            <div>
              <label className="block text-xs text-[#64748b] mb-1 font-medium uppercase tracking-wide">Evaluation Date</label>
              <input
                type="date"
          ...
          <div>
            <label className="block text-xs text-[#64748b] mb-1 font-medium uppercase tracking-wide">Narrative</label>
            <textarea
          ...
          <div className="space-y-3">
            {RATING_CATEGORIES.map((cat) => {
              ...
              if (total === 0) {
                return (
                  <div key={cat} className="flex items-center gap-3">
                    <div className="w-32 text-xs text-[#64748b] shrink-0">{cat}</div>
                    <div className="flex-1 text-[10px] text-[#94a3b8] italic">No ratings yet</div>
                  </div>
                );
              }
              const counts = {
              ...
              return (
                <div key={cat} className="flex items-center gap-3">
                  <div className="w-32 text-xs text-[#64748b] shrink-0">{cat}</div>
                  <div className="flex-1 flex h-5 overflow-hidden border border-[#e5e7eb]">
                  ...
                  <div className="w-10 text-[10px] font-mono text-[#94a3b8] text-right">{total}</div>
                </div>
              );
            })}
          </div>
          <div className="flex items-center gap-3 mt-4 pt-3 border-t border-[#e5e7eb] flex-wrap">
            {[
      ...
      {/* Ratings List */}
      {loading ? (
        <div className="text-center text-[#94a3b8] py-12">Loading ratings...</div>
      ) : ratings.length === 0 ? (
        <div className="max-w-[480px] mx-auto text-center p-12" style={{background: "linear-gradient(135deg, #fff1f2, #fff)", borderRadius: "16px", border: "1px solid rgba(225,29,72,0.08)"}}>
          ...
          <p className="text-sm text-[#64748b] mb-6">When you receive a CPARS evaluation from a contracting officer, enter it here. If any rating is below Satisfactory, the AI generates a formal response draft.</p>
          ...
          {ratings.map((r) => (
            <div key={r.id} className="border border-[#e5e7eb] bg-white p-5 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="text-sm text-[#0f172a] font-medium">
              ...
              <p className="text-sm text-[#64748b] mb-3">{r.narrative}</p>
      ...
      {responseView && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-full max-w-2xl border border-[#e5e7eb] bg-white p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="bg-[#f8f9fb] border border-[#e5e7eb] p-4 text-sm text-[#0f172a] whitespace-pre-wrap">
              {responseView.text}
            </div>
            <button
              onClick={() => setResponseView(null)}
              className="mt-4 w-full border border-[#e5e7eb] text-[#64748b] py-2 text-sm hover:border-[#d1d5db] transition-colors"
            >
              Close
            </button>
          </div>
